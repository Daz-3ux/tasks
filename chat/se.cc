...
int main(int argc, char *argv[])
{
  ...
  /*尽管我们分配了足够多的client_data对象，但为了提高poll的性能，仍然有必要限制用户的数据 */
  ...
  while (1)
  {
    ...
    for (int i = 0; i < num; i++)
    {
      ...
      // 监听套接字 + 读事件触发
      if (curfd == listenfd && (evs[i].events & EPOLLIN)) {
      ...
      // 两种套接字均可 + 异常触发
        ...
        if (getsockopt(curfd, SOL_SOCKET, SO_ERROR, &errors,
                    &length) < 0)
        {
      ...
      // 连接套接字 + 读事件触发
        ...
        else
        {
            ...
            {
                if (evs[j].data.fd == connfd)
                {
                    continue;
                }
                evs[j].events |= ~POLLIN;
                evs[j].events |= POLLOUT;
                users[evs[j].data.fd].write_buf = users[connfd].buf;
            }
        }
    // 连接套接字 + 写事件触发
    }else if (evs[i].events & POLLOUT) {
